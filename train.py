...
def inflate_weight_channel(weight, num_channels):
    # to get the number of channels, we look at the first conv layer
    for k in weight.keys():
        if ('cls_heads' in k) and ('cls_heads.6' not in k) and ('.bias' not in k):
            ...
            # compute the average weight for the last channel
            average_channel = np.mean(w, axis=0)
            print("avg",average_channel.shape)

            # copy the average channel weights to the inflated weights
...
def setup_training(args, net):
    ...
    num_steps = 224 * args.MAX_EPOCHS# Note: if you change the batch size, you need to change the num_steps
