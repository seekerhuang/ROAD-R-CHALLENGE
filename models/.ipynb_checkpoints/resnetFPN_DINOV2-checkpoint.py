...
class TSVideoSwinBackbone(nn.Module):
    ...
    def __init__(self,   pretrained=None,
        ...
        self.tconv=TimeGlobalMultiScaleBlock(1536,1536)
        ...
        self.conv2=BaseConv3DDINO(1536,768,3,1) # for 512 40 40
        self.deconv3=BaseTransConv3DDINO(1536,384,3,1)# for 256 80 80
        # self.norm1=nn.LayerNorm(1024)
        # self.norm2=nn.LayerNorm(512)
        # self.norm3=nn.LayerNorm(256)
        self.flowbackbone=viedoswinFlow()
        self.uptime1=nn.ConvTranspose3d(384,384,kernel_size=(2,1,1), stride=(2,1,1))
        self.uptime2=nn.ConvTranspose3d(768,768,kernel_size=(2,1,1), stride=(2,1,1))
        self.uptime3=nn.ConvTranspose3d(1536,1536,kernel_size=(2,1,1), stride=(2,1,1))
        self.catconv1=TSAttBlock(rgb_in_channels=384,flow_in_channels=384,out_channels=256,ksize=1,stride=1)
        self.catconv2=TSAttBlock(rgb_in_channels=768,flow_in_channels=768,out_channels=512,ksize=1,stride=1)
        self.catconv3=TSAttBlock(rgb_in_channels=1536,flow_in_channels=1536,out_channels=1024,ksize=1,stride=1)
    ...
    def forward(self, xrgb):
        ...
        # xflow3=self.uptime2(xflow[0][0])#768
        # xflow2=self.uptime1(xflow3)#384
        # xflow4=self.uptime3(xflow[0][0])#1536
        ...
        out_feat_1=out_feat
        out_feat_1=F.interpolate(out_feat_1, size=(out_feat_1.shape[2],16,22), mode='trilinear').contiguous()#1536
        out_feat_2=self.conv2(out_feat)
        out_feat_2=F.interpolate(out_feat_2, size=(out_feat_2.shape[2],32,44), mode='trilinear').contiguous()#768
        out_feat_3=self.deconv3(out_feat)
        out_feat_3=F.interpolate(out_feat_3, size=(out_feat_3.shape[2],64,88), mode='trilinear').contiguous()#384
...
modeldinobackbone=get_model('vit-giant-p14_dinov2-pre_3rdparty',device='cpu', pretrained=True,backbone=dict(out_type='cls_token_and_featmap')).backbone
