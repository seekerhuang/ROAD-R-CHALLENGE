...
class TSAttBlock(nn.Module):
    ...
    def forward(self, xrgb,xflow):
        """
        Forward pass of the TSAttBlock module.

        Args:
            xrgb (torch.Tensor): Input RGB tensor.
            xflow (torch.Tensor): Input flow tensor.

        Returns:
            torch.Tensor: Output tensor after passing through the TSAttBlock module.
        """
...
class TimeGlobalMultiScaleBlock(nn.Module):
    """ A basic multi-scale temporal convolutional block for video understanding.

        This block performs multi-scale temporal convolutions with kernel sizes of 3, 5, 7
        to capture temporal features at different scales.

        Args:
        in_channels (int): Number of channels in the input tensor.
        out_channels (int): Number of channels produced by the convolution.
        trans (bool): Whether to add a downsample and upsample convolution for
        downsampling the input.

        Overall, this block extracts hierarchical temporal features from videos while
        keeping the feature map size same or performing temporal downsampling. The
        multi-scale convolutions enrich the temporal modeling.
"""
...
modeldinobackbone=get_model('vit-giant-p14_dinov2-pre_3rdparty',device='cpu', pretrained=True,backbone=dict(out_type='cls_token_and_featmap')).backbone.cuda()
