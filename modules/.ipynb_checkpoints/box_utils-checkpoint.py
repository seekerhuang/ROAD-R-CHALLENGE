...
def nms(boxes, scores, overlap=0.5, top_k=20):
    ...
    while idx.numel() > 0:
        ...
        device = torch.device('cuda')#单卡
        xx1 = torch.Tensor().new_empty((idx.size(0),))
        yy1 = torch.Tensor().new_empty((idx.size(0),))
        xx2 = torch.Tensor().new_empty((idx.size(0),))
        yy2 = torch.Tensor().new_empty((idx.size(0),))
        x1 = x1.to(device)
        y1 = y1.to(device)
        x2 = x2.to(device)
        y2 = y2.to(device)
