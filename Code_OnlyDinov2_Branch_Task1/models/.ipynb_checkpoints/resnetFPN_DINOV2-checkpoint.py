...
from .yolox3dmodels.network_blocks import BaseConv3D, CSPLayer, DWConv,CSPLayer3D,BaseTransConv3DDINO,DWConv3D,BaseOnlyConv3D,BaseConv3DDINO,CSPLayer3DDINO
...
class TSAttBlock(nn.Module):
    def __init__(
        self, rgb_in_channels,flow_in_channels ,out_channels, ksize, stride, bias=False,tksize=3,tstride=1,zero_init_final_norm=True,zero_init_final_conv=True,non_local=False,uptemporal=False 
    ):
...
class TimeGlobalMultiScaleBlock(nn.Module):
    def __init__(self,in_channels,out_channels):
        ...
        nn.init.zeros_(self.tconv1.conv.weight)
        nn.init.zeros_(self.tconv1.conv.bias)
        nn.init.zeros_(self.tconv2.conv.weight)
        nn.init.zeros_(self.tconv2.conv.bias)
        nn.init.zeros_(self.tconv3.conv.weight)
        nn.init.zeros_(self.tconv3.conv.bias)
...
class TSVideoSwinBackbone(nn.Module):
    ...
    def __init__(self,   pretrained=None,
        ...
        self.tconv_1=TimeGlobalMultiScaleBlock(1024,1024)
        self.tconv_2=TimeGlobalMultiScaleBlock(512,512)
        self.tconv_3=TimeGlobalMultiScaleBlock(256,256)
        ...
        # self.flowbackbone=videomvit()
        # self.uptime1=BaseTransConv3DDINO(512,256,stride=2,ksize=2,tksize=1,tstride=1)#for 256 80 80
        # self.uptime2=BaseTransConv3DDINO(768,512,stride=2,ksize=2,tksize=2,tstride=2)
        # self.uptime3=BaseTransConv3DDINO(768,1024,stride=1,ksize=1,tksize=2,tstride=2)
    ...
    def forward(self, xrgb):
        # xflow=self.flowbackbone(xflow)
        # # xflow2=self.uptime1(xflow[0][0])
        # xflow3=self.uptime2(xflow[0][0])
        # xflow2=self.uptime1(xflow3)
        # xflow4=self.uptime3(xflow[0][0])
        # print("xflow2",xflow2.shape)
        # print("xflow3",xflow2.shape)
        # print("xflow4",xflow2.shape)
        cls_token,out_feat=self.rgbbackbone(xrgb)
        out_feat=self.tconv(out_feat)
        out_feat_1=out_feat
        out_feat_1=F.interpolate(out_feat_1, size=(out_feat_1.shape[2],16,22), mode='trilinear')#1024
        out_feat_2=self.conv2(out_feat)
        out_feat_2=F.interpolate(out_feat_2, size=(out_feat_2.shape[2],32,44), mode='trilinear')#512
        out_feat_3=self.deconv3(out_feat)
        out_feat_3=F.interpolate(out_feat_3, size=(out_feat_3.shape[2],64,88), mode='trilinear')#256
        out_feat_1=self.tconv_1(out_feat_1)
        out_feat_2=self.tconv_2(out_feat_2)
        out_feat_3=self.tconv_3(out_feat_3)
#         out_feat_1 = rearrange(out_feat_1, 'n c d h w -> n d h w c')
#         out_feat_1 = self.norm1(out_feat_1)
#         out_feat_1 = rearrange(out_feat_1, 'n d h w c -> n c d h w')# 1024 20 20

#         out_feat_2 = rearrange(out_feat_2, 'n c d h w -> n d h w c')
#         out_feat_2 = self.norm2(out_feat_2)
#         out_feat_2 = rearrange(out_feat_2, 'n d h w c -> n c d h w')# 512 40 40

#         out_feat_3 = rearrange(out_feat_3, 'n c d h w -> n d h w c')
#         out_feat_3 = self.norm3(out_feat_3)
#         out_feat_3 = rearrange(out_feat_3, 'n d h w c -> n c d h w')# 256 80 80
        # print("out_feat_2",out_feat_2.shape)
        # print("xflow2",xflow2.shape)
        # print("out_feat_3",out_feat_3.shape)
        # print("xflow3",xflow3.shape)
        # print("out_feat_1",out_feat_1.shape)
        # print("xflow4",xflow4.shape)
        # x2=self.catconv1(out_feat_3,xflow2)#256
        # x3=self.catconv2(out_feat_2,xflow3)#512
        # x4=self.catconv3(out_feat_1,xflow4)#1024
        # print("catconv1",x2.shape)
        # print("catconv2",x3.shape)
        # print("catconv3",x4.shape)
        return [cls_token,out_feat_3,out_feat_2,out_feat_1]
...
# config_path = '/root/miniconda3/lib/python3.8/site-packages/mmaction/.mim/configs/recognition/mvit/mvit-small-p244_k400-pre_16xb16-u16-100e_sthv2-rgb.py'
# checkpoint_path = '/root/autodl-tmp/road-dataset-master/ROAD-R-2023-Challenge-main_me/pretrainmodel/mvit-small-p244_u16_sthv2-rgb_20221021-65ecae7d.pth' # 可以是本地路径
