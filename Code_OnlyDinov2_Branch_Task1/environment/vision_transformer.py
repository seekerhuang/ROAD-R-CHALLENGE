...
class VisionTransformer(BaseBackbone):
    ...
    OUT_TYPES = {'raw', 'cls_token', 'cls_token_and_featmap','featmap', 'avg_featmap'}
    ...
    def _format_output(self, x, hw):
        ...
        if self.out_type=='cls_token_and_featmap':
            B = x.size(0)
            # (B, N, C) -> (B, H, W, C) -> (B, C, H, W)
            return x[:, 0], patch_token.reshape(B, *hw, -1).permute(0, 3, 1, 2)
        if self.out_type == 'avg_featmap':
