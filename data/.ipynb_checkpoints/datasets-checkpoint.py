...
class VideoDataset(tutils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.SEQ_LEN):
            ...
            images.append(img)
            # 读取optical flow图像   
            flow_name = self.flow_path + '/{:s}/{:05d}_flow.png'.format(videoname, frame_num+1)
            flow = Image.open(flow_name).convert('RGB')
            flows.append(flow)
        ...
        return clip,clipflow, all_boxes, labels, index, wh, self.num_classes, videoname, start_frame, is_unlabelled
...
def custom_collate(batch):
    ...
    for sample in batch:
        ...
        is_pseudo_labelled.append(sample[9])
    ...
    flows=get_clip_list_resized(flows)
    # print(images.shape)
    # return images, new_boxes, new_targets, torch.stack(ego_targets,0), torch.LongTensor(counts), image_ids, torch.stack(whs,0)
    return images,flows, new_boxes, new_targets, torch.LongTensor(counts), image_ids, torch.stack(whs,0), \
