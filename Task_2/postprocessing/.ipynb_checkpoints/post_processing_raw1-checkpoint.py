...
def main():
    ...
    parser.add_argument('--threshold', type=float, default=0.41, help='Threshold at which a label is considered as predicted. If label > threshold then it is considered predicted.')
    ...
    requirements_temp_path = f"{split_req_path[0]}_temp1.txt"
    ...
    with open(os.path.join(file_dir, f"post_processed0_1790_33_{os.path.basename(args.file_path)}"), 'wb') as handle:
