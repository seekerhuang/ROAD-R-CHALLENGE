...
def write_temp_file(preds, temp_file_path):

    # with open(wdimacs_path, 'r') as f:
    with open(temp_file_path, 'r+') as wf:
        # lines = wf.readlines()
        # offset = sum(len(line) for line in lines[:248])
        # print(offset)
        # Write the hard constraints
        # for i, line in enumerate(f.readlines()):
        #     wf.write(line)
...
def main():
    ...
    requirements_temp_path = "/root/autodl-tmp/road-dataset-master/ROAD-R-2023-Challenge-main_me/constraints/WDIMACS_requirements_temp4.txt"
    ...
    for videoname in preds.keys(): 
        ...
        for pred_framename in preds_framenames: 
            ...
            for i, bbox in enumerate(preds[videoname][pred_framename]):
                ...
                write_temp_file(pred_labels, requirements_temp_path)
    ...
    # 构造结果文件的相对路径
    # save_path = os.path.join(file_dir, f"post_processed_{os.path.basename(args.file_path)}")
    with open(os.path.join(file_dir, f"post_processed4_{os.path.basename(args.file_path)}"), 'wb') as handle:
