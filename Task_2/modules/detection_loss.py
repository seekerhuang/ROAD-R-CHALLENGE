...
class FocalLoss(nn.Module):
    ...
    def forward(self, confidence, predicted_locations, gt_boxes, gt_labels, counts, anchors, logic, Cplus, Cminus, is_ulb=None):
        ...
        with torch.no_grad():
            ...
            for b in range(gt_boxes.shape[0]):
                ...
                all_labels = torch.stack(all_labels, 0).float()  # shape seq_len x num_anchors(67536) x 42
                gt_locations = torch.stack(gt_locations, 0)   # shape seq_len x num_anchors(67536) x 4
                labels_bin = torch.stack(labels_bin, 0).float()   # shape seq_len x num_anchors(67536)
        ...
        gt_locations = gt_locations[pos_mask].reshape(-1, 4)  # shape num_pos x 4
        # print("gtloc",gt_locations)
        # print("---------------------------------------------------------")
        # print("pos_shape_afgt",pos_mask.shape)
        # print("---------------------------------------------------------")
        # print("gtshape",gt_locations.shape)
        # print("-------------------------------------------------------")
        # print("preloc",predicted_locations)
        # print("-------------------------------------------------------")
        # print("pos_shape_before_pre",pos_mask.shape)
        # print("-------------------------------------------------------")
        # print("preshape",predicted_locations.shape)
        predicted_locations = predicted_locations[pos_mask].reshape(-1, 4)  # shape num_pos x 4
        ...
        if all(is_ulb):
            masked_preds = preds.reshape(-1, self.num_classes)  # Remove Ignore preds  # TODO: check with example
        ...
        # masked preds shape: 538014 x 42
        if logic is None:
            return regression_loss, cls_loss/8.0  # + ego_loss/4.0
        else:
        ...
        return regression_loss, cls_loss / 4.0, logic_based_loss
