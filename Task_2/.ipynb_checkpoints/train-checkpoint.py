...
# def warmup_lr(args,epoch, warmup_iters):
#     if epoch < warmup_iters:
#         lr = (args.LR - args.WARMUP_LR) * float(epoch) / float(warmup_iters) + args.WARMUP_LR
#     else:
#         lr = args.LR
#     return lr
...
def setup_training(args, net):
    ...
    if args.RESUME > 0:
        ...
        net.load_state_dict(backbone_stat_dict)
...
#         checkpoint1 = torch.load(args.pretrained_model_path)
#         # for key in checkpoint1.keys():
#         #     print(key)
#         # print("------------------------------------------------------------------------")
#         state_dict = collections.OrderedDict(checkpoint1['state_dict'])
#         # for key in state_dict.keys():
#         #     print(key)
#         # print("------------------------------------------------------------------------")
#         # print("net is here",net)
#         # print("state_af_order",state_dict)
...
#         backbone_stat_dict = {}
#         # for keyt in net.module.keys():
#         #     print(keyt)
#         # torch.save(net.state_dict(), '/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_None_0.0/resnet50RCGRU512-Pkinetics-b12s16x1x1-roadt1-h3x3x3-09-21-12-09-21x/model_test_net.pth')
#         for k in state_dict.keys():
#             # 去掉backbone.
#             # new_k=k
#             if k.startswith('backbone.patch_embed'):
#                 backbone_stat_dict[k.replace('backbone.patch_embed', 'backbone.patch_embed')] = state_dict[k]
#             # 修改层名
#             if k.startswith('backbone.layers.0'):    
#                 backbone_stat_dict[k.replace('backbone.layers.0', 'backbone.swinaclayer1')] = state_dict[k]

#             if k.startswith('backbone.layers.1'):
#                 backbone_stat_dict[k.replace('backbone.layers.1', 'backbone.swinaclayer2')] = state_dict[k]
            
#             if k.startswith('backbone.layers.2'):
#                 backbone_stat_dict[k.replace('backbone.layers.2', 'backbone.swinaclayer3')] = state_dict[k]
            
#             if k.startswith('backbone.layers.3'):
#                 backbone_stat_dict[k.replace('backbone.layers.3', 'backbone.swinaclayer4')] = state_dict[k]
                
#             if k.startswith('backbone.norm3'):
#                 backbone_stat_dict[k.replace('backbone.norm3', 'backbone.norm3')] = state_dict[k]
...
#             if ('backbone.layer' in k) or ('conv6' in k) or ('conv7' in k) or ('lateral' in k) or ('corr_layer' in k) or ('heads' in k) or ('anchors' in k):
...
#                     backbone_stat_dict2[k.replace('module.', '')] = checkpoint2[k]
#                 if k.startswith('module.cls_heads'):
#                     backbone_stat_dict2[k.replace('module.', '')] = checkpoint2[k]
...
#         for k in list(state_dict_rgb.keys()):
       
#             # 修改层名
#             if k.startswith('backbone.patch_embed'):
#                 backbone_stat_dict_rgb[k.replace('backbone.patch_embed', 'backbone.backbone.rgbbackbone.patch_embed')] = state_dict_rgb[k]
#             # 修改层名
#             if k.startswith('backbone.layers.0'):    
#                 backbone_stat_dict_rgb[k.replace('backbone.layers.0', 'backbone.backbone.rgbbackbone.swinaclayer1')] = state_dict_rgb[k]

#             if k.startswith('backbone.layers.1'):
#                 backbone_stat_dict_rgb[k.replace('backbone.layers.1', 'backbone.backbone.rgbbackbone.swinaclayer2')] = state_dict_rgb[k]
            
#             if k.startswith('backbone.layers.2'):
#                 backbone_stat_dict_rgb[k.replace('backbone.layers.2', 'backbone.backbone.rgbbackbone.swinaclayer3')] = state_dict_rgb[k]
            
#             if k.startswith('backbone.layers.3'):
#                 backbone_stat_dict_rgb[k.replace('backbone.layers.3', 'backbone.backbone.rgbbackbone.swinaclayer4')] = state_dict_rgb[k]
                
#             if k.startswith('backbone.norm3'):
#                 backbone_stat_dict_rgb[k.replace('backbone.norm3', 'backbone.backbone.rgbbackbone.norm3')] = state_dict_rgb[k]
        ...
        for k in list(state_dict_flow.keys()):
            ...
            if k.startswith('backbone.layers.1'):
                backbone_stat_dict_flow[k.replace('backbone.layers.1', 'backbone.backbone.flowbackbone.swinaclayer2')] = state_dict_flow[k]
            
            if k.startswith('backbone.layers.2'):
                backbone_stat_dict_flow[k.replace('backbone.layers.2', 'backbone.backbone.flowbackbone.swinaclayer3')] = state_dict_flow[k]
            
            if k.startswith('backbone.layers.3'):
                backbone_stat_dict_flow[k.replace('backbone.layers.3', 'backbone.backbone.flowbackbone.swinaclayer4')] = state_dict_flow[k]
...
def train(args, net, train_dataset, val_dataset):
    ...
    for epoch in range(args.START_EPOCH, args.MAX_EPOCHS + 1):
        ...
        # lr = warmup_lr(args,epoch - 1, warmup_iters=2)
        if epoch ==18:
            for handler in logger.handlers:
                handler.close()
                logger.removeHandler(handler)
            log_file_path = '/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_Lukasiewicz_8.0/resnet50RCGRU512-Pkinetics-b15s16x1x1-roadt1t2t3-h3x3x3-10-23-22-54-45x/train-10-24-00-17-49-after18epo.log'
        ...
        if epoch ==19:
            ...
            log_directory = '/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_Lukasiewicz_8.0/resnet50RCGRU512-Pkinetics-b15s16x1x1-roadt1t2t3-h3x3x3-10-23-22-54-45x/'
            log_filename = 'train-10-24-00-17-49-after19epo.log'
        ...
        if epoch ==20:
            ...
            logger.setLevel(logging.INFO)
            log_directory = '/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_Lukasiewicz_8.0/resnet50RCGRU512-Pkinetics-b15s16x1x1-roadt1t2t3-h3x3x3-10-23-22-54-45x/'
            log_filename = 'train-10-24-00-17-49-after20epo.log'
            log_file_path = os.path.join(log_directory, log_filename)
            if not os.path.exists(log_directory):
                os.makedirs(log_directory)
            file_handler = logging.FileHandler(log_file_path, mode='w')
            ...
            #     handler.close()
            #     logger.removeHandler(handler)
            # log_file_path = '/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_Lukasiewicz_8.0/resnet50RCGRU512-Pkinetics-b15s16x1x1-roadt1t2t3-h3x3x3-10-23-22-54-45x/train-10-24-00-17-49-after19epo.log'
            # filehandler = logging.FileHandler(log_file_path)
            # logger.addHandler(filehandler)
            ...
            # for handler in logger.handlers:
            #       handler.close()
            # filehandler = logging.FileHandler('/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_Lukasiewicz_8.0/resnet50RCGRU512-Pkinetics-b15s16x1x1-roadt1t2t3-h3x3x3-10-23-22-54-45x/train-10-24-00-17-49-after18epo.log') 
            # logger.addHandler(filehandler)
            # filehandler.close()
            # filehandler.mode = 'w'
            # filehandler.stream = open('/root/autodl-tmp/road-dataset-master/SAVE/road/logic-ssl_cache_Lukasiewicz_8.0/resnet50RCGRU512-Pkinetics-b15s16x1x1-roadt1t2t3-h3x3x3-10-23-22-54-45x/train-10-24-00-17-49-after18epo.log', 'w')
        if epoch == 10:
            for param_group in optimizer.param_groups:
        ...
        iteration = run_train(args, train_data_loader, net, optimizer, epoch, iteration,scheduler,scaler)
...
def run_train(args, train_data_loader, net, optimizer, epoch, iteration,scheduler,scaler):
    ...
    for internel_iter, (mix_images, mix_gt_boxes, mix_gt_labels, mix_counts, mix_img_indexs, mix_wh, _, _, mix_is_ulb) in enumerate(train_data_loader):
        ...
        images = mix_images.cuda(0, non_blocking=False)
        # flows=flows=mix_flows.cuda(0,non_blocking=True)
        gt_boxes = mix_gt_boxes.cuda(0, non_blocking=False)
        gt_labels = mix_gt_labels.cuda(0, non_blocking=False)
        counts = mix_counts.cuda(0, non_blocking=False)
        ...
        iteration += 1
        if args.LOGIC is not None:
            Cplus = Cplus.cuda(0, non_blocking=False)
            Cminus = Cminus.cuda(0, non_blocking=False)
    ...
    torch.save(scheduler.state_dict(), '{:s}/scheduler_{:06d}.pth'.format(args.SAVE_ROOT, epoch))
    torch.save(scaler.state_dict(), '{:s}/scaler_{:06d}.pth'.format(args.SAVE_ROOT, epoch))
    # if epoch % 4 == 0:  # 当计数器的值能够被指定的参数整除时，执行保存操作
    #     logger.info('Saving state, epoch:' + str(epoch))
    #     torch.save(net.state_dict(), '{:s}/model_{:06d}.pth'.format(args.SAVE_ROOT, epoch))
    #     torch.save(optimizer.state_dict(), '{:s}/optimizer_{:06d}.pth'.format(args.SAVE_ROOT, epoch))
...
def run_val(args, val_data_loader, val_dataset, net, epoch, iteration):
        ...
        mAP_group = dict()
        with open('valmetrics.txt', 'a') as f:
            f.write('Epoch: {}\n'.format(epoch))
            for nlt in range(args.num_label_types):
                f.write('{} mAP: {:.5f}\n'.format(label_types[nlt], mAP[nlt]))
                for c, ap in enumerate(ap_all[nlt]):
                    f.write('{} {}: {:.5f}\n'.format(all_classes[nlt][c], ap))
